...
void AceleracionesV(uint np,ushort n_esp_p,uint nmaxvec,uint *M_int,uint *nvec,uint *vec,uint *esp_p,double &pot,double rc,double *M_param,double *pos,double *acel,double3 caja,bool nconf,potencial pote)
{
...
void SimulacionVec(float ncp,ushort n_esp_p,ushort n_esp_m,uint nc,uint np,uint max_p_en_esp_mr,uint GdL,uint *M_int, uint *esp_p,uint *n_p_esp_m,uint *n_m_esp_mr,uint *p_en_m,uint3 *mad_de_p,double dens,double dt,double temp_d,double p_termo,double rc,double *masa,
                    double *q_rat,double *pos,double *vel,double *acel,double *constr,double *param,double *dis_p_esp_mr_rep,double3 caja,ensamble ens,algoconstr vibrante,termostatos termo,potencial pot,std::ofstream &ofasres,std::ofstream &ofasat,std::ofstream &ofaeva)
{
    ...
    VecinosCercanos(np,nmaxvec,nvec,vec,mad_de_p,caja,pos,rc);
    ...
    AceleracionesV(np,n_esp_p,nmaxvec,M_int,nvec,vec,esp_p,epi,rc,M_param,pos,acel,caja,nconf,pot);
    ...
    for(int ic=0;ic<=nc;ic++)
    {
        ...
        if(ic%20==0)VecinosCercanos(np,nmaxvec,nvec,vec,mad_de_p,caja,pos,rc);
